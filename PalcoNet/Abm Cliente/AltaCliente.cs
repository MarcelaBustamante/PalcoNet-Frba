...
namespace PalcoNet.Abm_Cliente
{
	public partial class AltaCliente : Form
	{
		...
		public AltaCliente(dbmanager db, Decimal Cliente_id)
		{
			...
			existeR = this.db.Consultar("SELECT c.id, [Nombre],[Apellido],[Nro_documento],[CUIL],[Mail],isnull([Telefono],0) as Telefono,[Fecha_nacimiento],c.[Fecha_alta],d.Calle,d.Numero,d.Piso,d.Localidad,c.estado,d.Codigo_postal,t.Nro_tarjeta FROM[GD2C2018].[CAMPUS_ANALYTICA].[Cliente] c join CAMPUS_ANALYTICA.ClienteDireccion as cd	on cd.Cliente_id = c.Id join CAMPUS_ANALYTICA.Direccion  d on cd.Direccion_id = d.Id " +
												" join CAMPUS_ANALYTICA.Tajetas t on t.Cliente_Id = c.id where c.id = " + Cliente_id);
			if (existeR)
			{
				this.db.Leer();
				...
				tbNroTarjeta.Text = this.db.ObtenerValor("Nro_tarjeta");
			...
			{
				existeR = this.db.Consultar("SELECT c.id, [Nombre],[Apellido],[Nro_documento],[CUIL],[Mail],isnull([Telefono],0) as Telefono,[Fecha_nacimiento],c.[Fecha_alta],d.Calle,d.Numero,d.Piso,d.Localidad,c.estado,d.Codigo_postal FROM[GD2C2018].[CAMPUS_ANALYTICA].[Cliente] c join CAMPUS_ANALYTICA.ClienteDireccion as cd	on cd.Cliente_id = c.Id join CAMPUS_ANALYTICA.Direccion  d on cd.Direccion_id = d.Id  where c.id = " + Cliente_id);
				if (existeR)
				{
					this.db.Leer();
					tbNombre.Text = this.db.ObtenerValor("Nombre");
			...
			public static int CalcularDigitoCuil(string cuil)
			{
				...
				{
					MessageBox.Show("CUIL invalido: " + e.Message);
					return 1;
				}
			}
